public class Pedido {
    ...
    public void apresentarResumoPedido() {
        ...
        for(ItemPedido item: itens) {
            System.out.println(String.format("Tipo: %s  Titulo: %s  Preco: %.2f  Quant: %d  Total: %.2f", item.getProduto().getClass().getSimpleName(), item.getProduto().getTitulo(), item.getProduto().getPrecoBruto(), item.getQuantidade(), calcularValorPorItem(item)));
        }
        System.out.println("----------------------------");
